...
interface IEventPod {
    event CreatePredictEvent(
        uint256 requestId, string eventDescribe, string predictPosSide, string predictNegSid, address podAddress
    );

    event PredictEventResult(uint256 requestId, string winner, string predictPosSide, string predictNegSid);
    ...
    function fetchEventResult(uint256 _requestId)
        external
        view
